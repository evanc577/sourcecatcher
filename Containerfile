...
#
# Main Sourcecatcher image
#
FROM almalinux:9-minimal
LABEL org.opencontainers.image.source="https://github.com/evanc577/sourcecatcher"

WORKDIR /sourcecatcher
RUN microdnf install -y python3 python3-devel python3-pip libdb-devel gcc gcc-c++ redis findutils
...
RUN systemctl enable redis nitter sourcecatcher.service sourcecatcher-update.timer

COPY src/ /sourcecatcher/src/
COPY scripts/ /sourcecatcher/scripts/
RUN mkdir -p /sourcecatcher/images
...
CMD ["/sbin/init"] # Start systemd as PID 1
